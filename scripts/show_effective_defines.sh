    ...
    perl - "$C_PREFIX" "$HEADER_GUARD" "$TARGET" "$CONFIG_REAL" "$EFFECTIVE_FILE" > "$HEADER_TMP" <<'PERL'
        ...
        while (my $line = <$fh>) {
            ...
            if (defined $converted) {
                ...
                push @macros, [ $macro, $converted ];
            } else {
                print "/* $macro skipped: unsupported Verilog value: " . clean_comment($value) . " */\n";
            }
        }

        my $esp_printf = $prefix . "ESP_PRINTF";
        my $printf = $prefix . "PRINTF";

        print "\n";
        print "#define $esp_printf(...) ESP_LOGI(TAG, __VA_ARGS__)\n";
        print "#define $printf(...) printf( __VA_ARGS__)\n";
        print "\n";
        print "static inline int tt_macro_list(void)\n";
        print "{\n";
        print "    int err = 0;\n";

        for my $entry (@macros) {
            my ($macro, $converted) = @{$entry};

            print "#if defined($macro)\n";

            if ($converted =~ /^"/) {
                print "    $printf(\"$macro: %s\\n\", $macro);\n";
            } else {
                print "    $printf(\"$macro: %lu\\n\", (unsigned long)($macro));\n";
            }
